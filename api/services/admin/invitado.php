        ...
        switch ($_GET['action']) {
            case 'createRow':
                ...
                if (
                    ...
                    !$invitado->setId($_POST['id_prometido'])
                ) {
                    $result['error'] = $invitado->getDataError();
                } elseif ($invitado->createRowA()) {
                    $result['status'] = 1;
                ...
                break;
            case 'readAll':
                if (!$invitado->setId($_POST['id_prometido'])) {
                    $result['error'] = $invitado->getDataError();
                } elseif ($result['dataset'] = $invitado->readAllA()) {
                    $result['status'] = 1;
                } else {
                    $result['error'] = 'Los prometidos no tienen invitados registrados';
                }
                break;
            case 'readOne':
